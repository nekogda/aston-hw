...
class UserServiceImplTest {
    ...
    @Captor
    ArgumentCaptor<Supplier<User>> userCaptor;
    ...
    void createNewUserWhenSubmittedDataIsValid() {
        ...
        userService.create(TEST_USER_LOGIN, TEST_PASSWORD);
        verify(transactionManager).inTransaction(userCaptor.capture());

        Supplier<User> value = userCaptor.getValue();
        User createdUser = value.get();
    ...
    void create_throwsExceptionWhenLoginOccupied() {
        ...
        Assertions.assertThatThrownBy(() -> userCaptor.getValue().get())
                .isInstanceOf(UserAppValidationException.class)
    ...
    void changePassword_whenUserWasRecognized() {
        ...
        userService.changePassword(TEST_USER_LOGIN, TEST_PASSWORD, TEST_PASSWORD_NEW);
        verify(transactionManager).inTransaction(userCaptor.capture());

        Assertions.assertThatCode(() -> userCaptor.getValue().get())
    ...
    void changePassword_declineRequestWhenPasswordWasNotRecognized() {
        ...
        userService.changePassword(TEST_USER_LOGIN, TEST_PASSWORD_NEW, TEST_PASSWORD);
        verify(transactionManager).inTransaction(userCaptor.capture());
    ...
    void getAll_returnAllUsers() {
        when(repository.findAll())
                .thenReturn(List.of(getSavedUser()));
